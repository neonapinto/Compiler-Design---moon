...
public class LexicalAnalyser {
    ...
    public void writeToFiles(String file_path) {
        try {
            ...
            System.out.println("Reading and Writing Process ... ");
    ...
    private void add_transitions(int state, int[] next_state) {
        ...
        for (int i = 0; i < 20; i++) {
            transition_table[state].set_transition(characters[i], next_state[i]);
        }
    }
    ...
    public void createTransitionTable() {
        ...
        setFinalState(46, "operators", false);
        ...
        do {
            ...
            if (transition_table[curr_state].isFinalState()) {
                if ((transition_table[curr_state].isBacktrack()) && (lookup_char != '\n') && (lookup_char != '\r')) {
                    backtrack_char = lookup_char;              // back up
                } else {
                    backtrack_char = null;
